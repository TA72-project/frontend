...
export default function MissionsPage() {
  const [missionsPageStates, setMissionsPageStates] =
    useState<IMissionPageProps>({
      rows: [],
      paginationStates: {
        page: 1,
        perPage: 5,
  ...
  const handleDeleteMission = async (id: number) => {
    try {
      ...
      getAllMissions(
        missionsPageStates.paginationStates.page,
        missionsPageStates.paginationStates.perPage,
      ).then((value) =>
        setMissionsPageStates((prevState) => ({
          ...prevState,
          rows: value?.data,
        })),
      );
  ...
  return (
    <Box textAlign="center">
      ...
      />
      <Dialog onClose={handleOpenModal} open={openModal}>
        <DialogTitle>Créer ou modifier une mission</DialogTitle>
